@GIDEI @MVP @UIUX @IM @P1
Feature: [MVP] [UI/UX] [IM] Invoice Management Smoke Suite
  ...
  @TC_602480 @ModuleAccess @Admin @GIDEI_Smoke
  Scenario: Verify access to Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
  ...
  @ExportFunctionality @Admin @GIDEI_Smoke
  Scenario: Verify export functionality in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @FilterFunctionality @Admin @GIDEI_Sanity
  Scenario: Verify Column level functionality in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @TC_602537 @ApplyFilters @Outbound @Inbound @GIDEI_Smoke
  Scenario: Verify the application display the values in tables based on user selection made in filter criteria in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @InboundInvoicedetails @GIDEI_Smoke
  Scenario: Verify the application display the invoice details in Outbound invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @OutboundInvoiceExtractdetails @GIDEI_Sanity
  Scenario: Verify the application display the Outbound Invoice Extract in Outbound invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @OutboundInvoiceErrorDetails @GIDEI_Sanity
  Scenario: Verify the application display the Outbound Invoice Error details in Outbound invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @OutboundExport @GIDEI_Sanity
  Scenario: Verify the export functionality for Outbound Invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @OutboundFilter @ClearFilter @GIDEI_Sanity
  Scenario: Verify the Column level filter, Clear Filter and Reset view functionality for Outbound Invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @OutboundPagination @GIDEI_Sanity
  Scenario: Verify the Pagination functionality for Outbound Invoice template in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
  ...
  @InboundColumns @Inbound @GIDEI_Sanity
  Scenario: Verify all columns are displayed under Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    When I apply valid filter criteria in Invoice Management module
    Then all columns are displayed in Inbound Invoices (AP) grid

  @InboundFilter @ClearFilter @Inbound @GIDEI_Sanity
  Scenario: Verify the Status column filter and Clear Filter functionality for Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I click on Show Filter option in Inbound Invoices (AP) grid
    Then User is able to filter the records with Ready status in Inbound Invoices (AP) grid
    And I remove the Ready status records in Inbound Invoices (AP) grid
    Then User is able to filter the records with Error status in Inbound Invoices (AP) grid
    And I click on Clear Filter option to clear the applied filter
    Then User is able to clear the applied filter and all records are displayed in Inbound Invoices (AP) grid

  @InboundResetView @Inbound @GIDEI_Sanity
  Scenario: Verify the Reset View functionality for Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I click on Show Filter option in Inbound Invoices (AP) grid
    Then User is able to filter the records with Ready status in Inbound Invoices (AP) grid
    And I click on Reset View button to reset all filters applied
  ...
  @InboundExport @Inbound @GIDEI_Sanity
  Scenario: Verify the export functionality for Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I select records from Inbound Invoices (AP) grid
    Then I perform Download as Excel action on selected records and verify the downloaded file contains selected records in exported file
    Then I perform Download as CSV action on selected records and verify the downloaded file contains selected records in exported file
    Then I perform Download as JSON action on selected records and verify the downloaded file contains selected records in exported file
    Then I perform Download as XML action on selected records and verify the downloaded file contains selected records in exported file

  @InboundInvoiceDetails @Inbound @GIDEI_Smoke
  Scenario: Verify the application display the invoice details in Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I click on any Client invoice number column from Inbound Invoices (AP) grid
    Then the application displays the invoice details in Invoice details Pop up having EY logo with close button on top in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    Then I verify Invoice Header, Buyer, Seller,Totals & Payment,Line Items and Tax Summary section are getting displayed in invoice details pop up.
    And I click on Close button to close the invoice details pop up and verify the Invoice Management module is displayed with Outbound Invoice Template and Inbound Invoices (AP) sections

  @InboundInvoiceExtractdetails @Inbound @GIDEI_Sanity
  Scenario: Verify the application display the Inbound Invoice Extract in Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I Click on Status column to Filter Ready status records in Inbound Invoices (AP) grid
    Then I click on Ready status records from Inbound Invoices (AP) grid
    ...
    And I click on XML button to export Inbound extract in XML format
    Then I verify the exported file contains the correct data in Inbound Invoice Extract in XML format
    And I click on JSON button to export Inbound extract in JSON format
    Then I verify the exported file contains the correct data in Inbound Invoice Extract in JSON format
    And I click on CSV button to export Inbound extract in CSV format
    Then I verify the exported file contains the correct data in Inbound Invoice Extract in CSV format
    And I click on close button to close the Inbound Invoice Extract pop up and verify the pop up is closed.

  @InboundInvoiceErrorDetails @Inbound @GIDEI_Sanity
  Scenario: Verify the application display the Inbound Invoice Error details in Inbound Invoices (AP) grid in Invoice Management module for Admin role under Global Insights And Data Enrichment For e-Invoicing app in GTP IT
    When I navigate to Invoice Management module
    And I Click on Status column to Filter Error status records in Inbound Invoices (AP) grid
    Then I click on Error status records from Inbound Invoices (AP) grid
    ...
    And I click on close button to close the Inbound Invoice Error Details pop up and verify the pop up is closed.
